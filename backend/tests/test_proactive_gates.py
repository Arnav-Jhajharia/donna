...
async def test_ledger_returns_false_when_unseen():
    ledger = InMemoryDedupStore()
    assert await ledger.seen_async("u", "key", now=0.0) is False


async def test_ledger_marks_and_remembers():
    ledger = InMemoryDedupStore()
    await ledger.mark_async("u", "key", now=100.0)
    assert await ledger.seen_async("u", "key", now=120.0) is True


async def test_ledger_expires_after_ttl():
    ledger = InMemoryDedupStore(ttl_seconds=10.0)
    await ledger.mark_async("u", "key", now=100.0)
    assert await ledger.seen_async("u", "key", now=109.0) is True
    assert await ledger.seen_async("u", "key", now=111.0) is False


async def test_ledger_partitioned_by_user():
    ledger = InMemoryDedupStore()
    await ledger.mark_async("u_a", "key", now=0.0)
    assert await ledger.seen_async("u_b", "key", now=0.0) is False
...
async def test_apply_gates_drops_dedup_seen():
    ledger = InMemoryDedupStore()
    await ledger.mark_async("u", "watch:poke", now=0.0)
    moves = [_move(dedup_key="watch:poke")]
    outcome = await apply_gates(moves, user_id="u", ledger=ledger, now=1.0)
    assert outcome.accepted == []
    assert outcome.dropped[0].reason == "dedup: recently fired"


async def test_apply_gates_per_turn_budget():
    ledger = InMemoryDedupStore()
...
async def test_apply_gates_per_day_budget_with_used_count():
    ledger = InMemoryDedupStore()
...
async def test_apply_gates_daily_used_at_cap_drops_all():
    ledger = InMemoryDedupStore()
    moves = [_move(dedup_key="a"), _move(dedup_key="b")]
    outcome = await apply_gates(
        moves,
...
async def test_apply_gates_drops_relevance_failure():
    ledger = InMemoryDedupStore()
...
async def test_apply_gates_does_not_mark_ledger_on_accept():
    """Ledger marking happens after the worth-telling judge, not here."""
    ledger = InMemoryDedupStore()
    await apply_gates([_move(dedup_key="watch:x")], user_id="u", ledger=ledger)
    # Nothing was marked — gates are read-only on the ledger.
    assert await ledger.seen_async("u", "watch:x", now=0.0) is False
