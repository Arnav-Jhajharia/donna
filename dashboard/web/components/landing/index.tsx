...
export const Wordmark: React.FC<{
  ...
  const isMonogram = px < 14;

  // Hooks must run unconditionally (rules-of-hooks). The budget spend is
  // gated on both the color and whether we're rendering the full wordmark.
  const budget = useRustBudget();
  useEffect(() => {
    if (!isMonogram && color === 'rust') budget?.spend();
  }, [color, budget, isMonogram]);
