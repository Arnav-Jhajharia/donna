...
// Tailwind config for the landing-page surface only. The dashboard uses raw
// CSS and does not depend on Tailwind. We scope the content globs to the
// landing directories so Tailwind extracts classes only from there.
//
// The theme is closed (replace, not extend). Palette, type scale, and spacing
// scale come from the landing design system. Do not add arbitrary colors,
// arbitrary sizes, or new tokens here — if a landing screen needs something
// not in this list, the screen is wrong.

module.exports = {
  content: [
    './app/page.tsx',
    './components/landing/**/*.{js,jsx,ts,tsx}',
    './lib/landing/**/*.{js,jsx,ts,tsx}',
  ],
  theme: {
    colors: {
